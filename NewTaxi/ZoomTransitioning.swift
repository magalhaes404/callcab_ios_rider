...
extension ZoomTransitioning: UIViewControllerAnimatedTransitioning {
    ...
    private func animateTransitionForPush(_ transitionContext: UIViewControllerContextTransitioning) {
        ...
        UIView.animate(
            ...
            completion: { _ in
                ...
                self.destination.transitionDestinationDidEnd?(transitioningImageView: transitioningImageView as! UIImageView)
    ...
    private func transitioningPushImageView() -> UIView {
        let imageView = source.transitionSourceImageView()
        let frame = source.transitionSourceImageViewFrame(forward: forward)
        return UIView(baseImageView: imageView, frame: frame)
    }

    private func transitioningPopImageView() -> UIView {
        let imageView = source.transitionSourceImageView()
